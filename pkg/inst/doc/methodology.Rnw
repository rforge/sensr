...
The statistical methodology of sensory discrimination protocols is
...
\SweaveOpts{echo=f, results=hide, strip.white=t, width=4.5, height=4.5}
\SweaveOpts{prefix.string=./figs/methodology}
...
This document is devided into section that cover topics with similar
...
since they require that the nature of the difference (e.g. sweetnes)
...
protocols are used to test of un-specified differences. From a
...
strategy when using the undirectional protocols.  
...
response is prefered over another dispite the sensory distance remains
...
can be consistently prefered over another without affecting the
...
The four common sensory discrimination protocols are aften used in
...
between the products or samples, then the probability of a correct
...
--- we will treat these viewpoints synonimously throughout)
...
used synonimously thoughout, and they are actually based on the same
...
\begin{equation}
  ...
  p_c = f_{ps}(d')
\end{equation}
and expressions for the four common simple discrimination protocols
can be found in \citet{ennis93} and \citet{brockhoff10}.  
...
\begin{figure}
  \centering
<<psyFunsFig, fig = t, width=5, height=5>>=
...
@  
<<psyFunsFig_dprimePd, fig = t, width=5, height=5>>=
...
@ 
<<psyFunsFig_PcPd, fig = t, width=5, height=5>>=
...
@ 
<<psyFunFig_psyDeriv, fig = t, width = 5, height = 5>>=
...
There is a subtle but important destinction between the
...
appropriate levels:
<<rescaleExample, results = verbatim, echo = t>>=
...
\item \texttt{psyderiv}: implements the derivative of the psychometric
  functions, $f_{ps}'(d')$ for the four common discrimination.
\end{itemize}
...
Even when the parameter estimate is close, in some sense, to boundary
...
of the negative Hessian matrix (the matrix of second order derivaties)
...
models and almost always impressively successfull. 
...
their asymptotic behaviour is infact identical, their finite
sample properties may be quite different and often favour the
...
\subsubsection{Confidence intervald and hypothesis tests}
...
accompagnied by a two-sided confidence interval and one-sided
hypothesis tests should be accompagnied by one-sided confidence
...
The similarity tests discussed so far are targeted towards equivalence
...
assumpion that all assessors have the same probability of
...
<<rescaleExample2, results = verbatim, echo = t>>=
...
$\sqrt{\hat p_c (1 - \hat p_c) / n}$ as standard error in this
case. This is adopted in \texttt{discrim}. 
...
<<discrimExample1, results = verb, echo = t>>=
...
intervals and tests:
<<discrimExample2, results = verb, echo = t>>=
...
<<discrimExample3, results = verb, echo = t, fig = t, include = f>>=
...
information about the use and interpretation of (profile) likelhood
curves in sensory experiments is given in REFS.

\setkeys{Gin}{width=.49\textwidth}
\begin{figure}
  \centering
<<discrimExample3Fig, fig = t, include = t>>=
<<discrimExample3>>
@ 
...
likelhood based confidence intervals.}
...
number of correct answers that renders a test significant. 

In \textsf{R} we can find the $p$-values with
<<criticalValueExample1, results = verb, echo = t>>=
1 - pbinom(q = 15 - 1, size = 20, prob = 0.5)
1 - pbinom(q = 14 - 1, size = 20, prob = 0.5)
@ 

The while loop looks like
<<criticalValueExample2, results = verb, echo = t>>=
i <- 0
...
\textsf{sensR}: 
<<criticalValueExample3, results = verb, echo = t>>=
...
<<differencePowerExample1, results = verb, echo = t>>=
1 - pbinom(q = 15 - 1, size = 20, prob = 3/4)
@ 
or directly using the \texttt{discrimPwr} function from
\textsf{sensR}: 
<<differencePowerExample2, results = verb, echo = t>>=
discrimPwr(pdA = 0.5, sample.size = 20, alpha = 0.05, 
           pGuess = 1/2)
@ 
Observe that \texttt{discrimPwr} requires that the effect size under
the alternative hypothesis is given in terms of $d'$ rather than $p_d$
or $p_c$, so we needed to use the \texttt{psyinv} function to convert
from $p_{cA}$ to $d'_A$. 
...
<<differencePowerExample3, results = verb, echo = t>>=
...
<<similarityPowerExample1, results = verb, echo = t>>=
...
expression for the power of a test and calculation of power based
simulations can be an attactive approach. Sometimes it may also just
...
Recal that power is the probability of getting a significant result
...
of a bionomial parameter. The critical point above which the
...
determinition is in fact not as simple as the definitions may lead one
...
tests and below our target power. This is truely worrying; how many
...
at times piece-weise constant as a function of sample size,
cf. Fig.~\ref{fig:xcrAlpha}.  

\setkeys{Gin}{width=.49\textwidth}
\begin{figure}
  \centering
<<xcrFig, fig = t, height = 3.5, width = 5.5>>=
...
@ 
<<alphaFig, fig = t, height = 3.5, width = 5.5>>=
...
the same while lopp that could also be used to find the critical value
...
compromize is used, where the while loop is started at one if the
sample size estimated by the normal approximation is less than
50. Otherwise the while loop is started at 90\% of the normal
approximation estimate and sometimes even lower necessary. If the normal
approximation estimate is larger than 10,000, the function will
inform of that and not attemtp to estimate the sample size due to the
...
<<exactSSexample, results = verb, echo = t>>=
(pd <- coef(rescale(d.prime = .9, method = "triangle"))$Pd)
...
<<normalSSexample, results = verb, echo = t>>=
discrimSS(pdA = pd, pd0 = 0, target.power = 0.8, alpha = 0.05, pGuess
...
regarded as the extreme endpoints inbetween which the population of
...
Suppose 20 panellists each conduct 10 triangle tests, with $x_i$
...
fact one-sided, which motivates halfing the degrees of freedom.
...
difference will be supperior to the other two tests or at least at
least as good as them. 

\subsubsection{$\chi^2$-tests based on contingency tables}
\label{sec:chi2-tests-based-on-contingency-tables}


For the test of any difference (i.e. the no-difference test) an exact
binomial one-tailed difference test, cf. eg.~(\ref{eq:21}) may be
used. This test is only appropriate when the difference test is that of
no-difference. If the null hypothesis is at a $p_{c0} > p_g$, this
test is no longer valid. 

The Pearson $\chi^2$ test has test statistic
\begin{equation}
  \label{eq:43}
  X^2 = \sum_{jk} \frac{(o_{jk} - e_{jk})^2}{e_{jk}}
\end{equation}
and the often very similar likelihood ratio test has statistic
\begin{equation}
  \label{eq:45}
  G^2 = 2 \sum_{jk} o_{jk} \log \frac{o_{jk}}{e_{jk}} ,
\end{equation}
where $o_{jk}$ are the observed frequencies and $e_{jk}$ are the
expected frequencies in the two-way contingency table indexed by $j$
in rows and $k$ in columns. The reference distribution for these
statistics is a $\chi^2$-distribution with $(k-1)(j-1)$ degrees of
freedom. The likelihood ratio test is defined under zero-counts with
the definition: $0 \log 0 \equiv 0$.

Replicated binomial observations may be summarized in a $1\times 2$
table of correct and incorrect responses; $\sum_i x_i$ and $\sum_i
(n_i - x_i)$ respective. The expected number of correct answers under
the null hypothesis of no difference are $e_{11} = p_g n.$ and 
$e_{12} = (1-p_g)n.$, where $n. = \sum_i n_i$. Application of
(\ref{eq:43}) and (\ref{eq:45}) provide tests of the no-difference
hypothesis versus a general alternative. The tests are on one degree
of freedom.

The observations can also be arranged in a $N\times 2$ table with one
row for each individual ($i$). The Pearson and likelihood ratio tests
can also be applied to this table in a test of homogeneity (or
heterogeneity / over-dispersion). This test is on $(N-1)(2-1)$ degrees
of freedom. 

This corresponds to assuming that 
$X_i \sim \textup{binom}(p_{ci}, n_i)$, 
i.e. that each individual has a unique probability of a correct
response but with no model for these subject-specific
probabilities. In the beta-binomial model we augmented this model with
a model for the subject-specific probabilities. As $n_i$ increases,
and if $p_{ci}$ stay away from their boundaries, the beta-binomial
model converges to the ``fixed-effects'' model, i.e. without the
random (beta distributed) probability effects.

Many other models are possible, e.g. we could augment the binomial
model with the assumption that 
$\log (p_{ci} / 1 - p_{ci}) \sim N(p_c, \sigma^2)$, which leads to a
simple version of a generalized linear mixed model. However, all other
tests for homogeneity/over-dispersion mentioned here than that
provided by the chance-corrected beta-binomial are wrong in the sense
that 

<<replicatedTests, eval = f>>=
x <- c(2,2,3,3,rep(4, 7), 5, 6, 10, 11)
sum(x)
n <- rep(12, length(x))
sum(n)

chisq.test(c(sum(x), sum(n)))

x <- c(2, 2, 3, 4, 5, 5, 5, 5, 6, 6, 6, 7, 7, 7, 7, 7, 7, 8, 8, 8, 9,
       11, 12) 
n <- rep(12, 23)
(x. <- sum(x))
(n. <- sum(n))
discrim(x., n., method = "triangle")
(pc <- x./n.)
rescale(Pc = pc, std.err = sqrt(pc*(1-pc)/n.), method = "triangle")
## One df Pearson test:
(cst <- chisq.test(c(x., n. - x.), p = c(1/3, 2/3))) ## 49.32
## G^2 statistic:
(G2 <- 2 * with(cst, sum(observed * log(observed / expected)))) ## 46.16
pchisq(G2, df = 1, lower = FALSE)

summary(betabin(cbind(x, n), method = "triangle"))

(cst2 <- chisq.test(mat <- cbind(x, n-x))) ## sames as:
chisq.test(mat <- cbind(x, n-x), p = c(rep(1/3, 23), rep(2/3, 23)))

ra <- apply(mat, 1, function(x) x[1] / x[2]) 
sum(with(cst2, (observed - expected)^2 / expected))

G.test(mat)
names(cst)

## Similar test for over-dispersion:
summary(glm(cbind(x, n - x) ~ 1, family = binomial))
summary(glm(cbind(x, n - x) ~ 1, family = quasibinomial))

@ 

\begin{itemize}
\item simple binomial test (possibly weak)
\item the results of \citet{brockhoff03}
\item chi-square table approaches - refer to Meyners paper
\item beta-binomial model or chance-corrected beta-binomial model
  \begin{itemize}
  \item likelihood ratio test of any difference
  \item likeliood ratio test of over-dispersion
  \item appropriate degrees of freedom?
  \end{itemize}
\item How could we possibly do similarity testing?
\item Power and sample size considerations?
\end{itemize}

\subsection{Related literature}

\citet{meyners07c} Proposed to use a Pearson $\chi^2$ goodness-of-fit (GOF)
test to test for effects in a paired preference test. The idea is to
compare the observed freqencies with those of a binomial distribution
with $p = 1/2$. Any deviation from the binomial distribution is
evidence of heterogeneity in excess of that expected for the binomial
distribution, evidence of a mean difference in preference or some
combination of a mean effect and a variability effect. Meyners
proposed to use a simulation, or 'exact' version, of the test to have
an appropriate test when the expected frequencies are small. 

The test statistic is
\begin{equation}
  \label{eq:61}
  X= \sum_{j = 0}^k x_j = \sum_{j=1}^k \frac{(y_j - e_j)^2}{e_j} ,
\end{equation}
where $e_j = E[y_j]$ are the exected frequencies under the null model,
where $y_j \sim \textup{binom}(p = 1/2, k)$. This statistic is
asymptotically $\chi^2_k$ distributed under the null model. The exact
test is to compare the observed value of $X$ with a simulated
distribution under the null model. 

If $e_j$ are computed under, say, a beta-binomial model, \eqref{eq:61}
is a GOF test of the beta-binomial model. The degrees of freedom are
now $k - p$, where $p$ is the number of parameters under the model,
e.g. two under the beta-binomial model.

Using simulations, Meyners showed that the GOF test is more powerful
than the standard 'exact' binomial test and in general more powerful
than tests based on the beta-binomial model. He even observes this
when the observations are beta-binomially distributed. 

We note, however, that Meyners refer to an ``ordinary binomial test'',
by which we assume he means the two-sided so-called exact binomial
test (two-sided version of \eqref{eq:21}), but he is not precise about
this. 

Perhaps more worrying, he does not say which test based on the
beta-binomial model he employs; many tests, including for example 
Wald and likelihood ratio tests, are available.

One important point in \citet{meyners07c} is that a comparison of
observed and expected frequencies can lend insight into the structure
of preference in the studied population; are dispersion or mean
differences most important?

\citet{meyners07c} does not describe how many simulations he uses to
construct his tables of power comparisons. This makes it dificult to
assess the uncertainty in the numbers that he quotes. 

\begin{table}
  \begin{center}
    \caption{Half the 95\% CI width for various $n$ and $p$ to express the
      number of significant digits which should be reported for some
      no. simulations, $n$ and the probability parameter, $p$. This is
      relevant for simulations of power, coverage probability
      etc.\label{tab:ciWidth} } 
    \begin{tabular}{rrrrrrr}
      \hline
      & & \multicolumn{5}{c}{$n$} \\
      \cline{3-7} 
      \multicolumn{2}{c}{$p$} & $10^2$ & $10^3$ & $10^4$ & $10^5$ &
      $10^6$ \\  
      \hline
      0.1 & 0.9 & 0.05880 & 0.01859 & 0.00588 & 0.00186 &
      0.00059 \\  
      0.2 & 0.8 & 0.07840 & 0.02479 & 0.00784 & 0.00248 &
      0.00078 \\  
      0.3 & 0.7 & 0.08982 & 0.02840 & 0.00898 & 0.00284 &
      0.00090 \\  
      0.4 & 0.6 & 0.09602 & 0.03036 & 0.00960 & 0.00304 &
      0.00096 \\  
      0.5 & 0.5 & 0.09800 & 0.03099 & 0.00980 & 0.00310 &
      0.00098 \\  
      \hline 
    \end{tabular}
  \end{center}
...
In the implementation used by citet{meyners07c} the beta-binomial
model never converges more than 50\% of the cases when the
observations have standard binomial variation. When the oberservations
are over-dispersed, the Meyners' implementation of the beta-binomial
model rarely converges in 100\% of the cases. We believe this
behaviour to be highly implementation specific, and we expect that the
implementation of the beta-binomial model in package \textsf{sensR}
will only rarely diverge. If we are correct in our expectations, the
treatment of the ``beta-binomial approach'' in \citep{meyners07c} is
unfortunate, since it may lead some readers to believe that this is an
inherent feature of the model and not of a particular implementation. 
...
%% \item power for the no-difference test (exact binom)
%% \item power for general difference tests
%% \item power for general similarity tests
\item ISO standard power and sample size calculations
%% \item power computed from the normal approximation
%% \item power computed from simulations
\item sample size for
  \begin{itemize}
  \item simple no-difference tests
  \item general difference tests
  \item similarity tests
  \end{itemize}
%% \item Effect of a non-zero $p_d$ on the sample size of similarity
%%   tests
%% \item computational considerations
\item It is a problem that various arguments in the discrim
  family of functions are required to be provided on various scales;
  some in terms of $p_c$, some in terms of $p_d$, and some in terms of
  $d'$. I think we should strive for more consistency.
%% \item Further we should have a function that takes as argument the
%%   type of protocol and one of $p_c$, $p_d$ and $d'$ and returns values
%%   for the rest. This could include standard errors as an optional
%%   argument. DONE: \texttt{rescale} is now that function.
\end{itemize}


\bibliography{C:/Users/rhbc/Documents/DTU/EksProjPBB/EksProj}
%% \newpage
%% \appendix
%% \input{Appendix.tex}
...
plot(range(ss), c(0.73, .85), type = "n", xlab = "sample size",
     ylab = "power", axes = FALSE)
lines(ss, pwr, type = "l")
points(ss, pwr, pch = 20, cex = .8)
abline(h = 0.8, lty = 2)
...
plot(ss, xcr, type = "l")
points(ss, xcr, pch = 20)
1 - pbinom(113-1, 297, 1/3) # 0.04937999

n <- 299
1 - pbinom(findcr(n, p0 = 1/3) - 1, n, 1/3) # 0.04937999
aa <- sapply(ss, function(n)
             1 - pbinom(findcr(n, p0 = 1/3) - 1, n, 1/3))

plot(ss, aa, pch = 20, ylim = c(0.03, .05))
lines(ss, aa, lty = 1)

plot(ss, aa + 1 - pwr, type = "b")
plot(ss, pwr + 1 - aa, type = "b")

plot(ss, aa, type = "b")

1 - pbinom(112-1, 297, 1/3)
